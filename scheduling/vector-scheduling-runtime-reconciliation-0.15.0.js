(function () {
  ...
  const SENT_KEY = 'missionVectorRebelCoreCaseSent_v1';
  const STATUS_KEY = 'missionVectorReconciliationStatus_v1';
  const CSHIFT_ANCHOR = '2026-09-10';
  const CREDIT_CATEGORIES = new Set(['Firefighter', 'Swing', 'Tiller', 'TADE']);
  const MAX_BATCH = 50;

  if (window.top !== window.self || window.__mvciVectorReconciliation0150) return;
  window.__mvciVectorReconciliation0150 = { version: VERSION, startedAt: Date.now() };
  ...
  function status() { return loadJson(STATUS_KEY, { lastRunAt: null, lastSyncAt: null, lastError: null, openCases: 0 }); }
  ...
  function planCompatible(planCredit, segments) {
    ...
    if (planCredit === 'Swing') {
      // Swing/FF may look like Firefighter in Vector while still earning Swing credit.
      return roles.size === 1 && (roles.has('Swing') || roles.has('Firefighter'));
    }
  ...
  function totalHours(segments) {
    return segments.reduce((sum, s) => sum + (Number(s.durationHours) || 0), 0);
  }
  ...
  async function post(events) {
    const cfg = pairing(); if (!cfg) throw new Error('Rebel Core is not paired.');
  ...
  function eventFor(c) {
    return {
      kind: 'reconciliation_case', case_key: c.caseKey, shift_date: c.date, person_key: c.personId, person_name: c.personName,
      issue_type: c.issueType, status: c.status, summary: c.summary, evidence: c.evidence, proposed_credit: c.proposedCredit,
      resolution_credit: c.resolutionCredit, resolution_notes: c.resolutionNotes, source: c.source || `Vector Scheduling ${VERSION}`,
      created_at: c.createdAt, resolved_at: c.resolvedAt,
    };
  }
  ...
  async function syncCases() {
    ...
    const rows = Array.isArray(s.reconciliationCases) ? s.reconciliationCases : [];
    const pending = rows.filter(c => sent[c.caseKey] !== JSON.stringify([c.status,c.summary,c.proposedCredit,c.resolutionCredit,c.resolutionNotes,c.resolvedAt]));
    let count = 0;
    for (let i = 0; i < pending.length; i += MAX_BATCH) {
      const batch = pending.slice(i, i + MAX_BATCH);
      const result = await post(batch.map(eventFor));
      if (Number(result?.failed || 0) > 0) throw new Error(`Rebel Core rejected ${result.failed} reconciliation case(s).`);
      for (const c of batch) sent[c.caseKey] = JSON.stringify([c.status,c.summary,c.proposedCredit,c.resolutionCredit,c.resolutionNotes,c.resolvedAt]);
      saveJson(SENT_KEY, sent); count += batch.length;
    }
    setStatus({ lastSyncAt: nowIso(), lastError: null });
    refreshCard(); return { paired: true, sent: count };
  }

  function esc(v) { return String(v == null ? '' : v).replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c])); }
  function refreshCard() {
    const panel = document.getElementById('mvci-vs-panel'); if (!panel) return;
    let card = panel.querySelector('#vs-reconciliation-v0150');
    if (!card) { card = document.createElement('div'); card.id = 'vs-reconciliation-v0150'; card.className = 'vs-card'; panel.appendChild(card); }
    ...
    const st = status();
    card.innerHTML = `<h3>Reconciliation Ready Room <span class="vs-muted">${VERSION}</span></h3><div class="vs-muted" style="margin-bottom:7px">${open.length} open exception(s). Routine evidence stays out of your way; only unresolved/mismatched/partial cases land here.</div>${open.slice(0,5).map(c=>`<div style="border-top:1px solid #e2e7ed;padding:5px 0"><b>${esc(c.date)} · ${esc(c.personName)}</b><div class="vs-muted">${esc(c.issueType)} · ${esc(c.summary)}</div></div>`).join('')}<div style="display:flex;gap:6px;margin-top:7px"><button id="vs-reconcile-run-v0150" class="vs-btn secondary">Reconcile now</button>${pairing()?'<button id="vs-reconcile-sync-v0150" class="vs-btn secondary">Sync cases</button>':''}</div>${st.lastError?`<div class="vs-muted" style="color:#b42318;margin-top:6px">${esc(st.lastError)}</div>`:''}`;
    const runBtn = card.querySelector('#vs-reconcile-run-v0150'); if (runBtn) runBtn.onclick=()=>{try{const r=reconcile();alert(`Reconciliation complete.\n\n${r.open} open exception(s).`);}catch(e){setStatus({lastError:clean(e?.message||e)});alert(e?.message||e);}};
    const syncBtn = card.querySelector('#vs-reconcile-sync-v0150'); if(syncBtn) syncBtn.onclick=async()=>{syncBtn.disabled=true;try{const r=await syncCases();alert(`${r.sent||0} reconciliation case(s) synced to Rebel Command.`);}catch(e){setStatus({lastError:clean(e?.message||e)});alert(e?.message||e);}finally{syncBtn.disabled=false;}};
  }

  setInterval(refreshCard, 1800);
  setInterval(() => { try { reconcile(); } catch (_) {} }, 120000);
  setInterval(() => { if (pairing()) syncCases().catch(e => setStatus({lastError:clean(e?.message||e)})); }, 180000);
  setTimeout(() => { try { reconcile(); } catch (e) { setStatus({lastError:clean(e?.message||e)}); } }, 5000);
  setTimeout(() => { if (pairing()) syncCases().catch(() => {}); }, 15000);

  window.MVCI_VECTOR_RECONCILIATION_0150 = { version: VERSION, reconcile, syncCases };
})();
