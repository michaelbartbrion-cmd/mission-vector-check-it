(function () {
  ...
  if (window.top !== window.self || window.__mvciVectorActionPreview0220) return;
  window.__mvciVectorActionPreview0220 = { version: VERSION, startedAt: Date.now() };
  ...
  async function freshCensus() {
    ...
    if (!['sent-good','sent-partial'].includes(status.status)) throw new Error(`Fresh Vector census did not complete (${status.status || 'unknown'}).`);
    return status;
  }
  ...
  async function previewPackage(pkg) {
    ...
    try {
      const census = await freshCensus();
  ...
  function patchButtons() {
    ...
    const card = document.getElementById('vs-vector-bridge-v0200');
    if (card && !card.querySelector('#mvci-action-preview-note-v0220')) {
      const note = document.createElement('div');
      note.id = 'mvci-action-preview-note-v0220';
      note.className = 'vs-muted';
      note.style.marginTop = '6px';
      note.textContent = `Action preview ${VERSION} · read-only · automated Vector writes disabled`;
  ...
  window.MVCI_VECTOR_ACTION_PREVIEW_0220 = {
    version: VERSION,
    refreshWorklist,
    previewPackage,
    verifyPackage,
