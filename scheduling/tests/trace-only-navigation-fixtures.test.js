...
const location={origin:'https://www.crewsense.com',hash:'#2026-09-28',href:'https://www.crewsense.com/Application/ControlPanel/Schedule/#2026-09-28',assign(url){this.assigned=url},reload(){this.reloaded=(this.reloaded||0)+1}};
const history={state:{},replaceState(_state,_title,url){location.href=url;location.hash='#'+url.split('#')[1]}};
...
(async()=>{
 ...
 shown='2026-09-16';await ctx.openDate();assert.equal(location.reloaded,1);assert.equal(location.href,ctx.scheduleUrl(st.date));
 shown='2026-09-28';location.reloaded=0;delete location.assigned;await ctx.openDate();assert.equal(location.reloaded,0);assert.equal(location.assigned,undefined);
 location.hash='#2026/09/28';await ctx.openDate();assert.equal(location.reloaded,1);assert.equal(location.href,ctx.scheduleUrl(st.date));location.hash='#2026-09-28';
