...
// @description  Vector Rebel — This is the way. PPE inspections, Captain sets, local signatures, managed updates, and owner-only admin diagnostics
...
(function () {
    ...
    if (window.__vectorRebelInstance) {
        console.warn(
            `Vector Rebel 2.3.1: another instance (${window.__vectorRebelInstance.version || 'unknown'}) is already active on this page.`
        );
        return;
    }
    ...
    const POST_SUBMIT_HISTORY_GRACE_MS = 9000;
    ...
    function textLooksLikeCompletedInspectionEntry(text, modeTitle = '') {
        ...
        if (!normalized || normalized.length < 12 || normalized.length > 500) return false;
    ...
    function textHistoryEntryElements(modeTitle) {
        ...
        // Keep the smallest/leaf-most matching container so nested wrappers do
        // not inflate the completion count.
        ...
        const seen = new Set();
        const result = [];
        for (const entry of leaf.sort((a, b) => a.text.length - b.text.length)) {
            const key = uiText(entry.text);
    ...
    function modeHistoryEntries(item, modeTitle) {
        ...
        // Existing semantic table/grid path.
        for (const row of getModeHistoryRows(item, modeTitle)) {
            const text = uiText(row.textContent || '');
            if (text) entries.push({ element: row, text, source: 'table' });
        }

        // Actual observed Vector layout: completed history can be rendered as
        // ordinary text/card content lower on the asset page.
        for (const entry of textHistoryEntryElements(modeTitle)) {
            entries.push({ element: entry.el, text: entry.text, source: 'text' });
        }

        // De-duplicate exact normalized text while preserving one element to scroll to.
        const seen = new Set();
    ...
    function historyTextMultiset(values) {
        const map = new Map();
        for (const value of values || []) {
            const key = uiText(value);
            if (!key) continue;
    ...
    function addedHistoryEntriesSinceBaseline(run, item, modeTitle) {
        const before = historyTextMultiset(run.baselineHistoryEntries || []);
        const after = modeHistoryEntryTexts(item, modeTitle);
        const added = [];

        for (const text of after) {
            const key = uiText(text);
    ...
    function countModeCompletesOnAssetPage(item, modeTitle) {
        ...
        return modeHistoryEntries(item, modeTitle).length;
    }
    ...
    function completionEvidence(run, item) {
        ...
        // completed inspection-instance view, although this department's normal
        // PPE flow returns directly to the asset page.
        ...
        // after Submit, Vector returns to the exact asset page and adds a new
        // COMPLETE entry to the history lower on that page.
        if (
            ...
            if (
                ...
                const added = addedHistoryEntriesSinceBaseline(run, item, mode.title);
                const inspectorMatches = added.filter(text =>
                    uiIncludes(text, run.inspectorName) &&
                    /\bCOMPLETED?\b/i.test(text) &&
                    uiIncludes(text, mode.title)
                );

                if (inspectorMatches.length === 1) {
                    const auditText = inspectorMatches[0];
                    return `asset-history-plus-one-confirmed-${run.baselineCompleteCount}-to-${now}-inspector-${clean(run.inspectorName)}-text-${diagnosticTextHash(auditText)}`;
                }

                // Compatibility fallback for a semantic table whose exact row
                // text could not be diffed, but whose newest completed row is
                // still unambiguously this inspector.
                ...
                if (newest && uiIncludes(newest.text, run.inspectorName)) {
                    ...
                    const parsed = Number.isFinite(newest.parsedAt)
                        ? new Date(newest.parsedAt).toISOString()
                        : 'unparsed';
        ...
        // Weak safety fallback remains available only after history has had a
        // reasonable chance to render. It still never clicks Submit.
    ...
    async function openInspectionForCurrentAsset(run) {
        ...
        if (run.historyFallbackEnabled !== true && run.historyFallbackEnabled !== false) {
            try {
                ...
                run.baselineHistoryEntries = modeHistoryEntryTexts(item, mode.title);
                run.historyFallbackEnabled = true;
                run.historyBaselineWarning = '';
            } catch (error) {
