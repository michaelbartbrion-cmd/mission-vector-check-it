...
(function(){
'use strict';
if(window.top!==window.self||window.MVCI_WORK_EMAIL_READER_010)return;

const VERSION='work-email-reader-0.1.0-discovery';
const DEFAULT_ENDPOINT='https://base44.app/api/apps/6aa6b7634a031657377d4fad/functions/telemetryBridge';
const ROOT_ID='mvci-work-email-reader-v010';
...
function attr(el,n){return clean(el?.getAttribute?.(n)||'')}
function candidateRows(){
  const selectors=['[role="option"]','[data-convid]','[data-itemid]','[data-item-id]'];
  const found=[...document.querySelectorAll(selectors.join(','))].filter(visible);
  ...
  for(const el of found){
    ...
    const parentList=el.closest('[role="listbox"],[role="grid"],[aria-label*="message" i],[aria-label*="mail" i]');
    const hasMessageData=!!(attr(el,'data-convid')||attr(el,'data-itemid')||attr(el,'data-item-id'));
    if(!parentList&&!hasMessageData)continue;
    uniq.push(el);
  }
...
function parseRow(el,index){
  ...
  const unread=attr(el,'aria-selected')==='false'&&/unread/i.test(`${aria} ${el.className||''}`)||/\bunread\b/i.test(aria);
  ...
  if(lines.length){sender=lines[0]||'';subject=lines[1]||'';preview=lines.slice(2,5).join(' ')}
...
function folderName(){
  const selected=[...document.querySelectorAll('[aria-current="page"],[aria-selected="true"]')].filter(visible).map(x=>clean(x.getAttribute('aria-label')||x.textContent||'')).find(x=>/inbox|sent|draft|deleted|archive|junk|mail/i.test(x));
  return selected||'Outlook Web';
}
...
function snapshot(){
  const rows=candidateRows(),messages=rows.map(parseRow);
  return{rows,messages,folder:folderName(),mailbox:mailboxHint(),summary:{url:location.href,title:document.title,visibleCandidates:rows.length,listboxes:document.querySelectorAll('[role="listbox"]').length,grids:document.querySelectorAll('[role="grid"]').length}}
}

let state={open:false,busy:false,msg:'Discovery mode: nothing is captured until you press Capture visible list.',kind:'info',last:null};
function style(){if(document.getElementById(`${ROOT_ID}-style`))return;const s=document.createElement('style');s.id=`${ROOT_ID}-style`;s.textContent=`#${ROOT_ID}{position:fixed;right:16px;bottom:16px;z-index:2147483647;font-family:Arial,sans-serif;color:#263746}#${ROOT_ID} *{box-sizing:border-box}#${ROOT_ID} .orb{width:48px;height:48px;border-radius:50%;border:2px solid #204f6d;background:#fff;box-shadow:0 6px 20px #0004;font-weight:900;cursor:pointer}#${ROOT_ID} .p{width:390px;max-width:calc(100vw - 24px);margin-bottom:8px;background:#f6f8f9;border:1px solid #cbd8df;border-radius:10px;box-shadow:0 10px 30px #0005;overflow:hidden}#${ROOT_ID} .h{background:#173e63;color:#fff;padding:12px 14px;display:flex;justify-content:space-between;gap:10px}#${ROOT_ID} .b{padding:12px}#${ROOT_ID} button.a{width:100%;padding:10px;border-radius:8px;border:1px solid #173e63;background:#173e63;color:#fff;font-weight:700;cursor:pointer;margin-top:8px}#${ROOT_ID} button.s{padding:8px;border-radius:8px;border:1px solid #cbd8df;background:#fff;cursor:pointer}#${ROOT_ID} .m{margin-top:8px;padding:8px;border:1px solid #dce5ea;border-radius:8px;background:#fff;font-size:11px;line-height:1.4}#${ROOT_ID} .warn{margin-top:8px;padding:8px;border:1px solid #f0c36b;border-radius:8px;background:#fff8e7;font-size:10px;line-height:1.35}`;document.documentElement.appendChild(s)}
function render(){style();let r=document.getElementById(ROOT_ID);if(!r){r=document.createElement('div');r.id=ROOT_ID;document.documentElement.appendChild(r)}const snap=state.last;const c=cfg();r.innerHTML=`${state.open?`<div class="p"><div class="h"><div><b>WORK EMAIL READER</b><div style="font-size:10px;opacity:.75">${esc(VERSION)} · read-only discovery</div></div><button id="we-close" class="s">−</button></div><div class="b"><div style="font-size:11px"><b>Pairing:</b> ${paired()?'configured':'needed'} · <b>Visible candidates:</b> ${snap?.messages?.length??'not scanned'}</div><button id="we-scan" class="a">SCAN VISIBLE LIST · NO UPLOAD</button><button id="we-send" class="a" ${state.busy||!paired()?'disabled':''}>CAPTURE VISIBLE LIST</button><button id="we-cfg" class="s" style="margin-top:8px">Configure pairing</button><div class="m">${esc(state.msg)}</div><div class="warn"><b>Safety:</b> This script never clicks a message, never sends/replies, never deletes/archives/moves, and never changes read/flag state. Discovery captures list text only. Full message bodies are not implemented in 0.1.0.</div>${snap?`<details style="margin-top:8px"><summary style="font-size:11px;cursor:pointer">Discovery details</summary><pre style="font-size:9px;white-space:pre-wrap;max-height:180px;overflow:auto">${esc(JSON.stringify(snap.summary,null,2))}</pre></details>`:''}</div></div>`:''}<button id="we-orb" class="orb" title="Mission Vector Work Email">MAIL</button>`;
r.querySelector('#we-orb')?.addEventListener('click',()=>{state.open=!state.open;render()});r.querySelector('#we-close')?.addEventListener('click',()=>{state.open=false;render()});r.querySelector('#we-scan')?.addEventListener('click',()=>{state.last=snapshot();state.msg=`Discovery found ${state.last.messages.length} candidate message rows. Nothing uploaded.`;render()});r.querySelector('#we-cfg')?.addEventListener('click',configure);r.querySelector('#we-send')?.addEventListener('click',capture)}
...
async function capture(){state.busy=true;render();try{const snap=snapshot();state.last=snap;if(!snap.messages.length)throw new Error('No credible visible message rows were found. Discovery must be tuned before capture.');const capturedAt=new Date().toISOString(),captureId=`workmail:${Date.now()}:${Math.random().toString(36).slice(2,9)}`;const payload={version:VERSION,workEmailCapture:{captureId,capturedAt,source:'Outlook Web rendered UI',sourceVersion:VERSION,mailboxHint:snap.mailbox,folderName:snap.folder,visibleCount:snap.messages.length,quality:'partial',notes:'Discovery collector 0.1.0. List-only evidence; DOM mapping not yet live-validated. No mailbox mutations performed.',messages:snap.messages.map(({_diag,...m})=>m)}};const res=await request(payload);if(!res?.workEmail?.accepted)throw new Error(res?.workEmail?.error||res?.workEmail?.reason||'Rebel Command did not accept the capture.');state.msg=`Captured ${res.workEmail.messages??snap.messages.length} list rows to Rebel Command as PARTIAL discovery evidence. No mailbox state changed.`;state.kind='success'}catch(e){state.msg=`STOPPED: ${clean(e?.message||e)} No mailbox changes were made.`;state.kind='error'}finally{state.busy=false;render()}}

state.last=snapshot();render();
window.MVCI_WORK_EMAIL_READER_010={version:VERSION,snapshot,capture,configure,status:()=>({paired:paired(),messages:state.last?.messages?.length||0,url:location.href})};
})();
