...
(function(){
'use strict';
if(window.top!==window.self||window.MVCI_WORK_EMAIL_READER_012)return;
const VERSION='work-email-reader-0.1.2-discovery';
const DEFAULT_ENDPOINT='https://base44.app/api/apps/6aa6b7634a031657377d4fad/functions/telemetryBridge';
const ROOT_ID='mvci-work-email-reader-v012';
...
const iconish=s=>{const x=clean(s);return !x||x.length<=3||/^[^A-Za-z0-9]+$/.test(x)};
...
function parseRow(el,index){
  ...
  const meaningful=lines.filter(x=>!iconish(x));
  let sender='',subject='',preview='';
  if(meaningful.length){
    let i=0;
    while(i<meaningful.length&&timeLike(meaningful[i]))i++;
    sender=meaningful[i]||'';
    i++;
    while(i<meaningful.length&&timeLike(meaningful[i]))i++;
    subject=meaningful[i]||'';
    i++;
    const rest=meaningful.slice(i).filter(x=>!timeLike(x));
    preview=rest.slice(0,4).join(' ');
  }
